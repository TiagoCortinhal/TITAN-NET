...
class Titan(nn.Module):
    def __init__(self):
        ...
        self.resBlock1 = ResBlock(32, 2 * 32, 0.0, pooling=True, drop_out=False)
        ...
        self.upBlock1 = UpBlock(2 * 4 * 32, 4 * 32, 0.0)
        self.upBlock2 = UpBlock(4 * 32, 4 * 32, 0.2)
        self.upBlock3 = UpBlock(4 * 32, 2 * 32, 0.2)
        self.upBlock4 = UpBlock(2 * 32, 32, 0.0, drop_out=False)

        self.upsampling1 = Upsampling(conv, 2, 32)
        self.upResBlock1 = ResBlock(32, 32, 0.0, pooling=False, drop_out=False)
    ...
    def forward(self, z, cond):
        ...
        logits = self.upsampling1(up1e)
        logits = self.upResBlock1(logits)

        logits = self.logits(logits)

        logits = nn.Upsample(size=(376, 1241), mode='bilinear', align_corners=True)(logits)
